...
⚠️ 这两次改动**都还没打新 APK**，真机复验待做——`Assets/Resources/Fonts/` 的三档字重
加上去之后包体预计从 ~29 MB 涨到 ~35 MB。

### 2026-09-13：报表环形图 + 视图下拉 —— **待验收**
...
### 2026-09-13：图例色点 + 收入 tab 字色 + 记账页保留选项 —— **待验收**
...
**上表 13 项 + 前一节的 14 项全部未做**，等 Unity 里点 Play 走一遍再回来填结果。
对应提交见 `feat: 报表新增环形图视图，用下拉浮层切换`（前一节）与
`fix: 修报表页收入构成未选中时白底白字，并给环形图加图例色点`。

EditMode 那边 347 个用例全绿，但**测试全绿不代表界面验过**——这一版新增的界面部分里，
只有「建了什么节点、切视图有没有清场、回调有没有触发、配色常量对不对」是测试能给的，
位置、观感、清晰度，以及第 22-27 条那一整块记账页行为，都在上表里。
