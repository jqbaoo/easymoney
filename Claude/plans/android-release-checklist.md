...
1. **打一版新 APK 并在真机复验。** 上一版 APK 之后累计有六次改动没进包：
   三档字重、暖色纸感配色、月份条选月、环形图 + 视图下拉、图例色点与两处修正、
   **底部导航栏遮挡修复 + 锁竖屏**。包体预计从 ~29 MB 涨到 ~35 MB（字体是大头）。
   最后一项目前**只有真机能验**（编辑器里安全区是全屏、导航栏恒 0，改动完全不可见），
   清单见下面「真机待验收」一节
...
⚠️ **这一节前两轮已经走过真机了，剩下的还是待办**——走过两轮、各发现一个新问题，
第 19-28 项要在第三版 APK 上再走一遍。前两轮各自的结论见下面两个小节。
...
| 26 | 抄下屏幕上那行调试读数 | safeArea 的 y/h/w、屏幕尺寸、两个 navBar 取值、`vis`、算出的 extra |
