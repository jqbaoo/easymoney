...
| **底部导航栏遮挡修复** | `Core/Layout/*.cs`、`App/UI/AndroidSystemBars.cs`、`App/UI/SafeAreaFitter.cs`、`App/AppRoot.cs` | 计划外。真机上三键导航时**整块底部被盖住**（标签栏 + 账户页/报表页底部）。根因是 targetSdk 35 在 Android 15 上被强制 edge-to-edge，而 Unity 2022.3 的 `Screen.safeArea` 不包含导航栏（UUM-121413，只在 6.1 修复、没有回移）。修法是按差额补足而不是无条件减——详见第 8 节。同时**锁竖屏**（版式只验过竖屏，`matchWidthOrHeight = 0` 的前提也是宽度恒定）。**五轮真机各暴露一个新问题，都已收在代码里**：① 读数显示 API 30+ 的 `getInsets` 返回 0（已加回退）；② 底部空一条，根因是 Unity 那套主题画白图标；③ 导航键会自己消失 + 手势导航下标签栏悬着（关掉 `androidStartInFullscreen`、按 `tappableElement` 分辨导航模式）；④ 三键下**位置偏高**——窗口自己已经让开底部了，而 `getInsets` 照样报 124，于是重复预留（新增窗口间距判据）；⑤ 两条系统栏都是**纯黑**——`setStatusBarColor` / `setNavigationBarColor` 缺了 `FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS`，被系统静默忽略（先 `addFlags` 再设颜色）。见第 8 节与第 11 节。本轮 APK 仍含一行临时诊断读数，**第六轮真机验收尚未做** |
...
| **系统栏的底色调用会被静默忽略** | `Window.setStatusBarColor` / `setNavigationBarColor` 的文档写明：**只有窗口带 `FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS` 时才生效**。而 Unity 那套 `BaseUnityTheme` 继承自 Holo，**没有开这一位**——于是调用跑了、颜色被系统丢掉，真机上两条栏都是纯黑（第四轮用户直接问「为什么背景是黑色的」）。**不报错、不崩溃、日志里也什么都没有**，只调颜色的话会一直以为是自己参数传错了。⚠️ 同一轮截图里**顶部还有一条深色横带，那是临时诊断面板自己**（`Theme.SCRIM` 半透明黑底 + 白字），两个黑来源不同，别当成一个 | `AndroidSystemBars._ensureSystemBarsUsable()` 里**先 `addFlags(FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)` 再设颜色，顺序不能换**。诊断读数最后那行 `bars flags/sb/nb` 是判据：**屏幕上是黑的而读回来是页面底色 = 系统丢的**（那时该去改主题，而不是继续调这几个 API）；`flags` 里的 `04000000` / `08000000` 两位是半透明状态栏/导航栏，被谁设上了同样会让颜色不生效 |
