...
测试      ██████████ 100%   430 个用例全绿（Money 7 + Parser 17 + TimeUtil 15 + 建表 4 + 冒烟 1 + 构建配置 1 + 模型 4 + 分类仓储 16 + 账户仓储 12 + 账单仓储 10 + 校验 17 + 记账服务 11 + 筛选 18 + 报表 16 + 应用容器 7 + 快捷金额 6 + 界面工厂布局 3 + 账单展示 26 + 账户表单 13 + 报表展示 15 + 构建配置守卫 11 + 结构迁移 13 + 字体槽位 8 + 配色 10 + 选中态配色 7 + 月份条 10 + 月份弹窗 9 + 环形切分 14 + 视图清单 9 + 下拉列表 12 + 视图宿主 10 + 视图渲染 17 + 安全区换算 22 + 标签词表 4 + 名字匹配 10 + 一句话解析 28 + 草稿计划 17）
...
| **底部导航栏遮挡修复** | `Core/Layout/*.cs`、`App/UI/AndroidSystemBars.cs`、`App/UI/SafeAreaFitter.cs`、`App/AppRoot.cs` | 计划外。真机上三键导航时**整块底部被盖住**（标签栏 + 账户页/报表页底部）。根因是 targetSdk 35 在 Android 15 上被强制 edge-to-edge，而 Unity 2022.3 的 `Screen.safeArea` 不包含导航栏（UUM-121413，只在 6.1 修复、没有回移）。修法是按差额补足而不是无条件减——详见第 8 节。同时**锁竖屏**（版式只验过竖屏，`matchWidthOrHeight = 0` 的前提也是宽度恒定）。**五轮真机各暴露一个新问题，都已收在代码里**：① 读数显示 API 30+ 的 `getInsets` 返回 0（已加回退）；② 底部空一条，根因是 Unity 那套主题画白图标；③ 导航键会自己消失 + 手势导航下标签栏悬着（关掉 `androidStartInFullscreen`、按 `tappableElement` 分辨导航模式）；④ 三键下**位置偏高**——窗口自己已经让开底部了，而 `getInsets` 照样报 124，于是重复预留（新增窗口间距判据）；⑤ 两条系统栏都是**纯黑**——`setStatusBarColor` / `setNavigationBarColor` 缺了 `FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS`，被系统静默忽略（先 `addFlags` 再设颜色）。见第 8 节与第 11 节。**六轮真机验收全部通过**（第六轮 2026-09-14 确认两条系统栏都是页面底色、图标深色可辨），带读数的临时诊断组件已删 |
| **一句话记账（语音入口）** | `Core/Voice/*.cs`、`App/UI/PhraseInputDialog.cs`、`App/UI/PageBase.cs`、`App/AppRoot.cs`、`App/UI/Pages/RecordPage.cs` | 计划外新增。说一句「记一笔，账户现金，金额30，类型餐饮，备注哈哈哈」就把记账页填好。**分三层，规则全在 Core、页面只机械写控件**：`TransactionPhraseParser`（句子 → 名字，**标签锚定**：找准「金额」两个字，后面跟的就是金额——语序随意、空段无所谓、ASR 漏字也扛得住）、`DraftResolver`（名字 → Id，**先精确全等、再模糊回退**）、`DraftPlanner`（草稿 + 表单此刻的类型 → `DraftPlan`，说清楚这一句要改哪几项）。**「类型餐饮」与「类型收入」是同一个标签，靠值落在哪张表里区分**；分类按**这一句自己说出来的类型**找，不是表单当下选着的那个（否则「类型收入，分类工资」会被表单默认的「支出」带偏）。标签表在 `PhraseLabels`，逐字**最长优先**匹配，以后加标签不会把已有的短标签挤掉。入口是记账页标题栏右侧的按钮，页面用 `PageBase.HasHeaderAction` 自己声明要不要，显隐由 `AppRoot` 统一管；点开 `PhraseInputDialog` 输入——**`icon_mic.png` 还没画，眼下显示兜底文字「语音」，美术补图即生效、代码不用改**。⚠️ **ASR 还没接**（离线可用性未定、包体代价未定，先攒着「打字 / 粘贴」这条路，它把 NLU 那半段先立住）。⚠️ **两条真金白银的规则**：语音只**填表不保存**（最后那一下必须用户自己按），这一句**提到却没匹配上**的账户 / 分类一律**留空、不补默认值**——补了他会以为语音已经填对，直接保存就记到别的账户上，而界面上看不出任何异常。两条都钉在 `DraftPlannerTests` 里。**名字匹配容一个字之差**（`NameMatcher`，Levenshtein 编辑距离）：ASR 的中文错字以**同音替换**为主（「张三」→「张散」），原先的严格全等下错一个字整个字段就落空。取向是**宁可猜不中，不能猜错**，四道约束一起上——精确全等优先（重名取第一个，不受模糊规则约束）、长度差超一个字不认、距离必须最小**且唯一**（两个候选一样近时一个都不猜，留空用户一定看得见）、阈值随名字长度收紧（1 字不给容错，2-4 字改一个，5 字起改两个）。⚠️ **已知代价**：「工商银行」对「招商银行」只差一个字，与 ASR 的典型错法**完全同形**，约束层面区分不了，**会被认成同一个**；能区分的只有拼音（工 gong ≠ 招 zhao），那需要覆盖常用汉字的拼音表加多音字处理，收益撑不起代价，本次明确不做，另有一条用例把这个代价**显式钉住**（`PickUniqueIndex_DifferentEntitiesOneCharApart_AreStillMatched`）——留着它是为了让后人调阈值时能看见，而不是当成新 bug 去修。兜底仍是「填表不保存」那条规则：认错的结果写在表单上看得见，最后那一下得用户自己按。⚠️ `PhraseLabels` 的**标签词不做**模糊匹配——标签是语法词汇，切错会**改坏备注内容**，「金额」听成「金鹅」仍然整句失败，这是**有意保留**的。**新增 85 个用例**（标签词表 4 + 名字匹配 18 + 一句话解析 29 + 草稿计划 17 + 名字相似度 17）。**Play 验收通过**（2026-09-15，17 项 + 模糊匹配 3 项；验的是编辑器 Play，不是真机 APK）。**模糊匹配在真实库上验过**：「餐引」→「餐饮」端到端生效，「现全」因库里唯一账户叫「3」（1 字名不给容错）正确留空；⚠️ **「账户侧模糊命中后填表」这一格未验**——库里没有 2 字以上的账户名，补验只需建一个「现金」再说一次那句话。⚠️ 顺带一条教训：本轮清单的预期值照抄了计划里的示例数据（「账户现全 → 落现金」），**开验前读库才发现 `account` 表里只有一条叫「3」的记录**——差点据此去改一段本来正确的代码。清单见 `Claude/plans/android-release-checklist.md` |
...
│   ├── Scenes/                  场景（原型阶段用不到）
│   ├── Scripts/
│   │   ├── Core/                ✅ Money / MoneyParser / TimeUtil / Models / Queries / ValidationResult / TransactionValidator / Reports / Statements / Accounts / Typography / Layout / Voice（noEngineReferences）
...
│   │           ├── PhraseInputDialog.cs 一句话记账的输入弹窗（ASR 未接，眼下是打字）
...
- 安全区适配：`SafeAreaFitter` 比较 `Screen.safeArea` 与屏幕尺寸，变化才重算
  - **底部还要额外让开 Android 导航栏**，规则在 `Core/Layout/SafeAreaLayout`（纯函数，20 个用例钉着）
  - 让的是**差额** `Max(0, 底部预留高度 - safeArea.y)`，不是无条件减掉它。同一个
  ...
  - **两个来源，新 API 优先、它为 0 才回退老的**：`SafeAreaLayout.ResolveNavigationBarHeight`
    （同样是纯函数，5 个用例钉着）。真机实测（Redmi K60 / Android 15 / 三键导航）API 30+ 的
    `getInsets(Type.navigationBars())` 返回 **0**，而 deprecated 的
    `getSystemWindowInsetBottom()` 给出了正确的 **124px**——少了这条回退，`extra` 恒为 0，
  ...
  - **两条系统栏的底色与图标颜色**：`AndroidSystemBars.EnsureSystemBarsUsable()`。
    系统默认把两条栏画成**纯黑**，而本项目的底是燕麦米白，两条黑边夹着界面很扎眼；
    图标颜色则交给系统按 `windowLightNavigationBar` 决定，而 Unity 那套主题传下来的
    是「画白图标」——白图标落在燕麦米白底上就是隐形，底部那 124px 看着就是一块空白。
    所以这里做四件事，**顺序不能换**：① `addFlags(FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)`；
    ② 两条栏底色都设成 `Theme.BACKGROUND`；③ `setSystemBarsAppearance` 把两条栏的图标
    都改成深色；④ `show(navigationBars())` + `BEHAVIOR_DEFAULT`（别自动隐藏）。
    **只在 API 30+ 动手**，理由见第 11 节
    - ⚠️ **少了第 ① 步，第 ② 步会被静默忽略**：`setStatusBarColor` /
      `setNavigationBarColor` 的文档写明只有窗口带这个标志时才生效，而 Unity 那套
      Holo 主题没有开它——代码跑了、颜色被系统丢掉，真机上就是两条纯黑，
      **不报错、不崩溃**。这也是第四轮真机上两条栏都是黑的原因
    - 调用点在 `SafeAreaFitter._apply()`，**每次重算安全区都重申一遍**，不是启动时调一次。
      Unity 的播放器自己会给窗口设全屏沉浸标志（`unity.launch-fullscreen` →
      `SYSTEM_UI_FLAG_HIDE_NAVIGATION | IMMERSIVE_STICKY`），设的时机在我们后面，
      只调一次会被它盖掉——现象是导航键「露一下、慢慢消失、划一下才浮出来、一两秒又缩回去」
    - **Player Settings 里的「Start in Fullscreen Mode」必须是关的**（`androidStartInFullscreen: 0`）。
      勾上等于自己请求全屏沉浸，导航键就常驻不了。`AndroidPlayerSettingsTests.StartInFullscreen_IsDisabled` 守着
...
| **导航栏图标是白的，画在浅色底上等于隐形** | Unity 生成的 `BaseUnityTheme` 在 API 31+ 继承 `android:Theme.Holo.Light.NoActionBar.Fullscreen`，而 **Holo 是 API 27 之前的东西，压根没有 `windowLightNavigationBar` 属性**，取默认值 `false` = 让系统画**白色图标**。Android 15 之前不显形（导航栏是不透明黑条，白图标配黑底正好），edge-to-edge 把导航栏变成没有底色的浮层之后才暴露——白图标落在燕麦米白底上就是看不见。真机现象：底部空出 124px 什么都没有，而读数 `nav 124` 说明系统认为导航栏正占着那 124px（真被藏起来会变 0） | 经 `WindowInsetsController` 设 `APPEARANCE_LIGHT_NAVIGATION_BARS`（深色图标）+ `show(navigationBars())` + `BEHAVIOR_DEFAULT`（别自动隐藏），见 `App/UI/AndroidSystemBars.EnsureSystemBarsUsable()`。**只在 API 30+ 动手**——Android 11 以下还是不透明黑条，在那里设「浅色导航栏」会变成黑图标画黑底。`WindowInsets.isVisible(navigationBars())` 就是用来分「系统藏了」和「画了但看不见」的（**当轮的临时读数把它打成了 `vis` 位**，那个组件已删） |
...
| **系统栏的底色调用会被静默忽略** | `Window.setStatusBarColor` / `setNavigationBarColor` 的文档写明：**只有窗口带 `FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS` 时才生效**。而 Unity 那套 `BaseUnityTheme` 继承自 Holo，**没有开这一位**——于是调用跑了、颜色被系统丢掉，真机上两条栏都是纯黑（第五轮用户直接问「为什么背景是黑色的」）。**不报错、不崩溃、日志里也什么都没有**，只调颜色的话会一直以为是自己参数传错了。⚠️ 同一轮截图里**顶部还有一条深色横带，那是当时那版临时诊断面板自己**（`Theme.SCRIM` 半透明黑底 + 白字；该面板已随第六轮验收删掉），两个黑来源不同，别当成一个 | `AndroidSystemBars._ensureSystemBarsUsable()` 里**先 `addFlags(FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS)` 再设颜色，顺序不能换**。判据是**读回窗口带的标志位与两条栏的颜色**（`getAttributes().flags` / `getStatusBarColor()` / `getNavigationBarColor()`）：**屏幕上是黑的而读回来是页面底色 = 系统丢的**（那时该去改主题，而不是继续调这几个 API）；`flags` 里的 `04000000` / `08000000` 两位是半透明状态栏/导航栏，被谁设上了同样会让颜色不生效 |
