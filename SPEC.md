...
| **一句话记账（语音入口）** | `Core/Voice/*.cs`、`App/UI/PhraseInputDialog.cs`、`App/UI/PageBase.cs`、`App/AppRoot.cs`、`App/UI/Pages/RecordPage.cs` | 计划外新增。说一句「记一笔，账户现金，金额30，类型餐饮，备注哈哈哈」就把记账页填好。**分三层，规则全在 Core、页面只机械写控件**：`TransactionPhraseParser`（句子 → 名字，**标签锚定**：找准「金额」两个字，后面跟的就是金额——语序随意、空段无所谓、ASR 漏字也扛得住）、`DraftResolver`（名字 → Id）、`DraftPlanner`（草稿 + 表单此刻的类型 → `DraftPlan`，说清楚这一句要改哪几项）。**「类型餐饮」与「类型收入」是同一个标签，靠值落在哪张表里区分**；分类按**这一句自己说出来的类型**找，不是表单当下选着的那个（否则「类型收入，分类工资」会被表单默认的「支出」带偏）。标签表在 `PhraseLabels`，逐字**最长优先**匹配，以后加标签不会把已有的短标签挤掉。入口是记账页标题栏右侧的按钮，页面用 `PageBase.HasHeaderAction` 自己声明要不要，显隐由 `AppRoot` 统一管；点开 `PhraseInputDialog` 输入——**`icon_mic.png` 还没画，眼下显示兜底文字「语音」，美术补图即生效、代码不用改**。⚠️ **ASR 还没接**（离线可用性未定、包体代价未定，先攒着「打字 / 粘贴」这条路，它把 NLU 那半段先立住）。⚠️ **两条真金白银的规则**：语音只**填表不保存**（最后那一下必须用户自己按），这一句**提到却没匹配上**的账户 / 分类一律**留空、不补默认值**——补了他会以为语音已经填对，直接保存就记到别的账户上，而界面上看不出任何异常。两条都钉在 `DraftPlannerTests` 里。**新增 59 个用例**（标签词表 4 + 名字匹配 10 + 一句话解析 28 + 草稿计划 17）。**Play 验收通过**（2026-09-15，17 项；验的是编辑器 Play，不是真机 APK），清单见 `Claude/plans/android-release-checklist.md` |
