...
| `chevron_left.png` | 上一月 / 弹窗里上一年 | 40 | 文字 `<` |
| `chevron_right.png` | 下一月 / 行尾箭头 / 弹窗里下一年 | 40 / 44 | 文字 `>` |
| `chevron_down.png` | 月份条上年月文字右边的下拉提示 | 28 | 文字 `↓` |
...
⚠️ **兜底字符只能挑字体子集里真有的字**（子集范围见上面「字体」一节，脚本
`D:\font-tmp\subset.py`）。挑错了代码不报错、测试也测不出来——**真机上渲染成空白**，
跟备注里的 emoji 是同一个坑。已实测过的结果：
...
要新增一个子集里没有的兜底字符，先改脚本重跑子集化，别直接写进代码。
